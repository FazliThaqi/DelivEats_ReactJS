...
import { SD_Categories } from "../../Utility/SD";
  ...
  //variable for categories  , it stores all of them
const Categories = [
...
function MenuItemUpsert() {
  //const id - needed for edit menu items
  const { id } = useParams();
  const navigate = useNavigate();
  //first const - will be the image that we want to store in the database
  //next one , what is the image we want to display
  ...
  //this useEffect will be called data
  useEffect(() => {
    if (data && data.result) {
      const tempData = {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      //that file we can use when we actually create the menu item
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("SpecialTag", menuItemInputs.specialTag ?? "" ); //Empty String is stored when posting and if special tag is null
        ...
        <div className="row mt-3">
          <div className="col-md-7">
            ...
            >
              ...
              {/* Mapping through the category and getting the value */}
